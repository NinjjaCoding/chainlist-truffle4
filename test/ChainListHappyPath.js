var ChainList = artifacts.require("./ChainList.sol"); // of course we need this to cmmunicate

// test suite
contract('ChainList', function(accounts){ // again these tests are against accounts
  var chainListInstance; //we need to save the instances for the records and to check values against
  var seller = accounts[1]; //again we must identify each account
  var buyer = accounts[2];
  var articleName1 = "article 1"; //this is for the sell item function
  var articleDescription1 = "Description for article 1"; //needs space for description
  var articlePrice1 = 10;
  var articleName2 = "article 2"; //this is for the buy function
  var articleDescription2 = "Description for article 2";
  var articlePrice2 = 11;
  var sellerBalanceBeforeBuy, sellerBalanceAfterBuy; //we need to check buyer and seller accounts before and after the buy
  var buyerBalanceBeforeBuy, buyerBalanceAfterBuy;// to make sure the funds are there and correct money transfered

  it("should be initialized with empty values", function() { //of course the intial values must be empty
    return ChainList.deployed().then(function(instance) { //as usual get the deployed instance
      chainListInstance = instance; //save it
      return chainListInstance.getNumberOfArticles(); //we are testing there is no articles for sale to do that
    }).then(function(data) { //we need to access data --then
      assert.equal(data.toNumber(), 0, "number of articles must be zero"); //assert the getNumberOfArticles is = 0,
      return chainListInstance.getArticlesForSale(); //return that value from chainListInstance getArticlesForSale global variable
    }).then(function(data){ //make sure that the total length of the dat = 0
  ...
  it("should let us sell a first article", function() {
    return ChainList.deployed().then(function(instance){ //get a deployed isntacne of our contract
      chainListInstance = instance; //save it
      return chainListInstance.sellArticle( // return to user name, description and price of articel and save it sellArticle folder
        articleName1, //seller must provide the this the follwoing items
        articleDescription1,
        web3.toWei(articlePrice1, "ether"),
        {from: seller}
      );
    }).then(function(receipt){ //then assert or make sure the follwoing aseertions are met
      // check event
      assert.equal(receipt.logs.length, 1, "one event should have been triggered"); // of couse we need
      assert.equal(receipt.logs[0].event, "LogSellArticle", "event should be LogSellArticle"); //i will have to look inot these but i think this just logs the infor
